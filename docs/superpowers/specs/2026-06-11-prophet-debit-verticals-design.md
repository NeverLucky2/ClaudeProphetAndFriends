...
**Deterministic backstops** (fire regardless of the LLM). **Precedence as implemented and tested (Phase 1):** the **near-expiry both-legs-OTM branch evaluates first** — at/under force-close DTE with the entire spread OTM, either `let_expire` (residual ≤ expected exit cost) or `force_close`; only then **salvage stop → profit-capture → force-close** as the catch-all. (The carve-out must preempt salvage: anything cheap enough to let expire is always already under the salvage floor, so a salvage-first ordering would make the carve-out unreachable and pay a pointless bid/ask round-trip near expiry.) The backstops:
